...
docker build -t gomithriltsapp:1.0 .

# Run the app docker container.
docker run -d -p 8081:8081 -e MYSQL_HOST=host.docker.internal -e MYSQL_ROOT_PASSWORD=password --rm --name gomithriltsapp-api gomithriltsapp-api:1.0
